...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"divide_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f divide-*.tar.gz
	echo Create archive \"divide-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../divide-$(VERSION)-$(BRANCH).tar.gz divide
...
divide.tar.gz: build
	cd make_build; tar czf ../divide.tar.gz divide && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./divide make_build
	cp *.md make_build/divide/
